...
int main(void) {
    ...
    // 'posix_spawn' also lets us create our own environment variables
    // We are just going to reuse this program's environment rather than make a new one
    char **my_env = environ;
    ...
    int ret = posix_spawn(&pid, "./file_size", NULL, NULL, my_argv, my_env);
    //  ^                 ^     ^                          ^        ^
